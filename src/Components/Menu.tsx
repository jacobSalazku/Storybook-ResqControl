...
import { IconArmchair, IconCheck, IconRefresh, IconReport } from "@tabler/icons-react";
...
const Menu = () => {
    ...
    const activeNavIcon = (index: number) => (select == index ? "#F6F8FF" : "#497DAE");
    const activeNav = (index: number) => (select == index ? selectedStyle : bgColor);
    ...
    return (
        <div className="my-1 flex w-full flex-row items-center justify-center gap-3 ">
            ...
            <MenuItem title={navTitles[2]} icon={<IconRefresh color={activeNavIcon(2)} />} backgroundColor={activeNav(2)} onClick={() => handleSelection(2)} />

            <MenuItem title={navTitles[3]} icon={<IconCheck color={activeNavIcon(3)} />} backgroundColor={activeNav(3)} onClick={() => handleSelection(3)} />
        </div>
    );
};
